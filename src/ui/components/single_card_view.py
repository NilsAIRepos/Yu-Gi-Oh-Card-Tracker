...
def setup_high_res_image_logic(img_id: int, high_res_remote_url: str, low_res_url: str, image_element: ui.image, current_id_check: Callable[[], bool] = None):
    """
...
def render_consolidated_single_view(
    card: ApiCard,
    total_owned: int,
        ...
        with ui.dialog().props('maximized transition-show=slide-up transition-hide=slide-down') as d, ui.card().classes('w-full h-full p-0 no-shadow'):
            ...
            with ui.row().classes('w-full h-full no-wrap gap-0'):
                ...
                with ui.column().classes('w-1/3 min-w-[300px] h-full bg-black items-center justify-center p-8 shrink-0'):
                    ...
                    # Initial image setup
                    setup_high_res_image_logic(img_id, high_res_url, low_res_url, image_element)
                    ...
                    def update_image(new_img_id):
                        ...
                        setup_high_res_image_logic(new_img_id, h_res, l_res, image_element, current_id_check=lambda: True)
...
def render_collectors_single_view(
    card: ApiCard,
    owned_count: int,
    set_code: str,
    rarity: str,
